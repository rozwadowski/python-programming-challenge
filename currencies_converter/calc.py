...
def main():
    code = ["PLN"]
    val = [1.0]
    urls = ["http://api.nbp.pl/api/exchangerates/tables/a/today/?format=json","http://api.nbp.pl/api/exchangerates/tables/b/?format=json"]
    for u in urls:
    ...
    try:
        ...
        while True:
            ...
            if len(command) != 3:
                ...
                if (not command[1].upper() in code) or (not command[2].upper() in code):
                    print "Blad wyboru walut" 
                else:
                    calc(float(command[0]),code.index(command[1].upper()),code.index(command[2].upper()),val,code)

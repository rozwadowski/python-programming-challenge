...
class Maze(object):
    def __init__(self):
        ...
        while len(Frontier) > 0:
            ...
            cl = choice(a)
            while cl not in Cells:
                cl = choice(a)
            Cells.append(cF)
            for i in range(g.degree()[cF]):
                if not g.vs[cF].neighbors()[i].index in Cells
                and not g.vs[cF].neighbors()[i].index in Frontier:
                    Frontier.append(g.vs[cF].neighbors()[i].index)
            g.delete_edges((cl, cF))
            Frontier.remove(cF)

        # plotting maze
        draw.rect(self.window, (255, 255, 255), Rect(40, 40, N*20, N*20), 1)
        for i in g.es:
            x1 = i.tuple[0] % N
            y1 = i.tuple[0] / N
            x2 = i.tuple[1] % N
            ...
            if y1 == y2:
                draw.line(self.window, (255, 255, 255),
                          ((x1+x2)/2*20+60, y1*20+40),
                          ((x1+x2)/2*20+60, y1*20+60), 1)
            if x1 == x2:
                draw.line(self.window, (255, 255, 255),
                          (x1*20-40, (y1+y2)/2*20+60),
                          (x1*20+60, (y1+y2)/2*20+60), 1)

        draw.rect(self.window, (0, 255, 0),
                  Rect(45, 45, 10, 10), 0)
        draw.rect(self.window, (0, 0, 255),
                  Rect(45+20*(N-1), 45+20*(N-1), 10, 10), 0)
        ...
        x2 = visitor2 % N
        y2 = visitor2 / N
        draw.line(self.window, (255, 165, 0),
                  (x1*20+50, y1*20+50), (x2*20+50, y2*20+50), 1)
        i = len(self.vis) - 3
        while visitor2 != 0:

...
class polygon(object):
    ...
    def rotate(self, x, y, a):
        ...
        for i in range(len(self.vertex)):
            self.vertex[i] =
            [self.vertex[i][0]*cos(a)-self.vertex[i][1]*sin(a),
             self.vertex[i][0]*sin(a)+self.vertex[i][1]*cos(a)]
        self.translate(x, y)

    def draw(self):
        for i in range(len(self.vertex)):
            plt.plot([self.vertex[i][0], self.vertex[i-1][0]],
                     [self.vertex[i][1], self.vertex[i-1][1]], 'b-', lw=2)

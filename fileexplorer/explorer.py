    ...
    def screendraw(self):
        ...
        for el in self.list:
                if os.path.isdir(self.path+"/"+el):
                    ...
                    window.blit(text,(20,self.size*(self.list.index(el)+1-self.shift)))
                if os.path.isfile(self.path+"/"+el):
                    text = Font.render(el[:25],True,(0,255,0))
                    window.blit(text,(20,self.size*(self.list.index(el)+1-self.shift)))
        text = Font.render("<----",True,(0,0,255))
        window.blit(text,(20,0))
        draw.rect(window,(106,106,106),Rect(20,self.ind*self.size,200,self.size),1)
        ...
        if picked > 0:
            if self.list[picked-1][-4:] in [".jpg",".png"] :
                ...
                if photo.get_width() > res[0]-200:
                    ...
                    photo=transform.scale(photo,(res[0]-200,scale*photo.get_height()))
                if photo.get_height() > res[1]:
                    scale = photo.get_height()/(res[1])
                    photo=transform.scale(photo,(photo.get_width(),res[1]*scale))

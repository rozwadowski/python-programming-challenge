...
class oButton(object):
	...
	def draw(self,game):
		if np.sqrt((mouse.get_pos()[0]-self.x)**2+(mouse.get_pos()[1]-self.y)**2) <= self.r:
			...
			draw.circle(game.window, (255,255,255), (self.x,self.y), self.r, 1)
...
class Game(object):
	...
	def event(self):
		if not self.gameover:
			if not (self.snake[0][0]+self.dir[0] == self.apple[0] and self.snake[0][1]+self.dir[1] == self.apple[1]):
			...
			for i in range(1,len(self.snake)):
				if self.snake[i][0]==self.snake[0][0] and self.snake[i][1] == self.snake[0][1]:
					self.gameover = True
			if self.snake[0][0] < 0 or self.snake[0][0] >= self.X or self.snake[0][1] < 0 or self.snake[0][1] >= self.Y:
				self.gameover = True
	...
	def draw(self):
		...
		for i in range(self.X):
			for j in range(self.Y):
				draw.rect(self.window,(20,20,20),Rect(i*self.size,j*self.size,self.size-2,self.size-1),0)
		for s in self.snake:
			draw.rect(self.window,(20,255,20),Rect(s[0]*self.size,s[1]*self.size,self.size-2,self.size-1),0)
		draw.rect(self.window,(255,20,20),Rect(self.apple[0]*self.size,self.apple[1]*self.size,self.size-2,self.size-1),0)
			
		text = self.Font.render("Points: "+str(self.points),True,(255,255,255))
		self.window.blit(text,(50,630))
		if self.gameover:
			text = self.Font.render("GAME OVER. Press n to start new game",True,(255,255,255))
